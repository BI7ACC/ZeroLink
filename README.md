...
1.  ✅ **定义 `ChatBlock` 数据结构与数据库存储层接口。**
2.  实现端到端加密模块 (`/core/crypto`)。
3.  实现消息链的本地存储 (`/core/storage`)，支持区块的追加和验证。
4.  实现引导服务器 (`/server/bootstrap`) 和客户端的 `Hole Punching` 逻辑。
5.  实现P2P直连通信，能够发送和接收 `ChatBlock`。
6.  实现群聊的广播和消息同步协议 (`SYNC_REQUEST`, `SYNC_RESPONSE`)。
7.  实现私聊的离线消息机制。
8.  实现 Peer Relay 和 Server Relay 作为回退方案。
